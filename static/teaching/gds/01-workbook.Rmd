...
You can see that the predictions can go a little bit funky... If your friend waits for one minute at the Milk Thistle, there is a *negative probability* that the wait will be too long:

```{r}
scenario = data.frame(pub='The Milk Thistle', waiting=1)
predict(wait_model2, scenario)
```
Having a negative percent chance of something happening is [against the law](https://en.wikipedia.org/wiki/Probability_axioms#First_axiom), and I don't want you to get busted for having contraband lying around, so let's try something a little better than this.
