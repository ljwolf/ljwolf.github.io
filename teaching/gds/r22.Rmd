...
Now; this can be a *good* model, or it can be a *bad* model; it's just one of many. We can see that the *average* prediction miss of this model is (basically) zero^[remember, scientific notation means that `r mean(residuals(model_1))` has sixteen zeros to the right of the decimal before hitting that first 6, so it's `.00000000000000006`], since regression is an *unbiased* predictor:
```{r}
mean(residuals(model_1))
```

An *improvement* to this guess might consider which pub someone waited in. If we were to just think about the waiting times *for that pub*, we might have better predictions. 


This can give us a *better* prediction:
```{r}
...
times['drinktype'] = ifelse(impatience %% 2 > 1, "Cocktail", "Beer")
...
```
which is spooky and also convenient.^[and based on the fact that the *expectation* is a fancy version of the weighted average!]
...
Looking at those lines, you know that the residuals are going to have some odd structure: $y$ can only be zero (if the wait is not too long) or one (if the wait is too long). This means that the residual, usually $y - \hat{y}$, will instead be $0-p_i$ or $1 - p_i$, since $\hat{y}$ in this model is a "predicted probability that $y_i$ is one" ($p_i$), and $y_i$ is only ever zero or one. While it's possible for these residuals to be distributed normally in abstract, it's unlikely in practice. This is *not* what we want to see for linear regression, so we have to be *very* careful about interpreting this model. 
