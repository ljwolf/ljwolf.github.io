...
sales %>% filter(property_type=="flat") %>% 
  group_by(is_freehold) %>% summarize(n())
```
## 1.4 
...
```{r}
yearly_price_model = lm(price ~ as.factor(year), data=sales)
...
```{r}
sales %>% mutate(residual = residuals(yearly_price_model)) %>%
  ggplot(aes(x=year, group=year, y=residual)) + geom_boxplot()
```
...
```{r}
freehold_model = glm(is_freehold ~ price, data=sales, 
                     family=binomial())
```
...
```
*To imnprove upon this, use the `predict` function to get probabilities back for model observations. That is, moving from a price of £100k to a price of £200k, the probability of being a freehold listing goes from 71% to 74%.*
```{r}
...
```{r}
with(sales %>% mutate(predict_prob = predict(freehold_model, type='response')), 
     table(actual = is_freehold, predicted = predict_prob > .5))
```
...
```{r echo=F}
accuracy = function(threshold, percent=TRUE, data=sales, model=freehold_model){
  xt = with(data %>% 
              mutate(predict_prob = predict(model, type='response')), 
            table(actual = is_freehold, predicted = predict_prob > threshold))
  n_right = sum(diag(xt))
  return(ifelse(percent, n_right/nrow(data), n_right))
}
```
*The $\hat{p}>.75$ model, which correctly predicts * `r accuracy(.75)`*percent of the data, compared to *`r accuracy(.5)` *for the $.5$ model and* `r accuracy(.99)` *for the $.99$ model.*
...
```{r}
accuracy = function(threshold, percent=TRUE, 
                          data=sales, 
                          model=freehold_model){
  xt = with(data %>% mutate(predict_prob = predict(model, type='response')), 
                             table(actual = is_freehold, predicted = predict_prob > threshold))
  n_right = sum(diag(xt))
  return(ifelse(percent, n_right/nrow(data), n_right))
}
