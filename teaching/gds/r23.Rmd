...
1. using `caret`, train the following models:
...
2. using `caret`, train the following binary classifiers:
...
3. using `caret`, train the following multi-class classifiers:
   ...
   - using the same crossvalidation setup, train a *multinomial logistic* model (`method='multinom'`) to predict the genre of a song based on the same covariates.
   - which has the best *in-sample* accuracy? which has the worst?
   - if you use preprocessing (`preprocess=c('center', 'scale')`), do results change?
