...
Often, autoregressive models are used in modeling spatial data or time series data. Often, building in
...
**Markov Property:** *For a process with outcome $Y$ observed over $t$ peroids in time, a* **Markov process** *of order $p$ is one where the outcome at time $t$ depends only on periods $p$ in the past, and no further:* 
$$P(Y_t = y_t | Y_{t-1} = y_{t-1}, Y_{t-2} = y_{t-2}, \dots, Y_0 = y_0) = P(Y_t = y_t | Y_{t-1} = y_{t-1}, Y_{t-2} = y_{t-2}, \dots, Y_{t-p} = y_{t-p})$$
Thus, the probability of observing a specific value of $Y_t$ only depends on $p$ periods in the past, even though you may actually know more than $p$ periods into the past. Often, in practical situations, $p=1$, but it may not necesssarily be so.[^0]
...
hi.to.low = ((scores[1:26] > mean(scores)) * (socres[2:27] < mean(scores)))
...
This property, that a stable Markovian process converges to a unique *limiting distribution* is a powerful property. In general, it's what powers *Markov chain Monte Carlo* methods, those used inside of software you've probably used before to fit any kind of model in a Bayesian framework. If you're interested, [I have a demonstration of this for the simplest linear regression](http://ljwolf.org/teaching/gibbs/). 
...
2. Two of these would be known as "divergent" or "explosive" processes. This means that, in the limit, their variance is infinite, and they do not converge to a time-invariant limiting distribution. For the distributions you made in part 1:
    ...
    - Now, plot each sequences' "cumulative mean," which is the mean of all observations seen up until that iteration. Then, plot their cumulative standard deviations.[^4]
    - Which sequences have a variance that appear to converge to a finite number? Do any converge exactly to $\sigma$?
    - Do you notice anything unique about the sequences that are explosive and those that are not? [^5]
...
Thus, we usually restate these kinds of models in a vector specification. So, let a $N$-vector of responses $Y$ exist and a matrix $W$ containing the elements $w_{ij}$. Together we use these to state a common autoregressive model, the simultaneous autoregressive lag model:
$$ Y = \rho \mathbf{W} Y + \epsilon$$
This is a direct analogue of the AR(p) process. But, note that we now *dont* specify an "order" parameter for this process, the determination of $\mathbf{W}$ is considered "fixed." It is uncommon to introduce a specific parameter to $\mathbf{W}$ that governs its order.

## The spatial lag polynomial

Indeed, this is common because the "final" structure of the spatial autoregressive lag model contains higher-order contributions regardless of the structure of $\mathbf{W}$. First, though, we must reduce this model. Recall how we changed the AR(p) statement from being about $Y_{t-k}$ to being a potentially-inifinite accumulation of errors. 
...
$$ Y = (I - \rho \mathbf{W})^{-1} \epsilon = (I + \rho \mathbf{W} + \rho^2 W^2 + \dots)\epsilon = \sum_{i=0}^\infty\rho^i\mathbf{W}^i\epsilon$$
Thus, again, there is an equivalence between the accumulating errors and autoregressive statments. Both are equivalent. Further, we might think that $\rho$ again should again be bound between $-1$ and $1$ in order the sum to vanish as $i$ increases. However, since $\rho$ is mediated by $W^i$, it is necessary both that $W$ and $\rho$ exhibit special properties in order for this to be stable.

This model is usually fit using maximum likelihood methods, but can also be fit using Bayesian methods, or multi-stage least squares algorithms, depending on the assumed variance structure of $\nu$.
...
which induces $Y$ to take this spatial correlation structure as well. 

## Exercise 3.1

### What is not autoregressive?

In general, there are many spatial [mixed-effect](https://books.google.co.uk/books?hl=en&lr=&id=WVHRBQAAQBAJ&oi=fnd&pg=PP1&dq=hierarchical+models+banerjee+carlin+gelfand&ots=AouZxmQNr6&sig=O8Dc0SFK31ZYaSLLg97bZ1CvQt0) [structures](https://www.jstor.org/stable/30045248), and many spatially-varying slopes models in multilevel settings (even if they're not continuously-varying) can be thought of as having spatial correlation i ntheir errors or effects. So, consider a typical varying-slope multilevel model. Why is that not autoregressive by itself? What makes a stochastic process autoregressive?[^10]

*(Specifying and estimating CAR models is a robust and at times contetious literature, somewhat unlike the simultaneous literature where the model and specification is relatively well-behaved and well-defined. For a good recent discussion of fitting a conditional autoregressive model in this style, I suggest consulting [Spatial models in Stan](http://mc-stan.org/users/documentation/case-studies/icar_stan.html))*
...
library(ggmap)
library(readr)
bklyn = ggmap::get_map(location='Brooklyn, NY', zoom=12)
...
- First, fit a model using:
    log(price) ~ accommodates + bathrooms + bedrooms + days_since_last_review + host_is_superhost + instant_bookable + review_scores_rating + has_pool + has_wifi + has_beach + is_private + has_pets
...
\end{align}
Note that this implies that a change to *a single site*, $X_i$, causes changes in *only that $\hat{Y}_i$. Do you think this is the case for simultaneous spatial lag model?[^12] To be clear, refer to the model discussed above with the form:
...
Second, there are significant stability issues, since space and time reverberate. To notice this, consider a STAR(1,1), which is a typical spatial lag model with a single term from the past also being relevant:
...
[^4]: *hint for Exercise 2.1 part 2: you could do the standard deviation with a result vector `sdeviations` using the following loop: `for(i in 10:length(outcome)){sdeviations = c(sdeviations, sd(outcome[1:i]))}`*
